...
function StrainsPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (strains.some((s) => s.code === code)) return toast.error("Code already exists");
  ...
  return (
    <div className="p-4 space-y-4 max-w-4xl">
      ...
      <Card className="p-4 bg-card border-border space-y-2">
        {strains.map((s) => {
          const count = units.filter((u) => u.strainCode === s.code).length;
          return (
            <div key={s.code} className="flex items-start gap-3 border border-border rounded p-3 bg-secondary/20">
              <div className="flex-1 min-w-0">
                <div className="font-mono text-sm">#{s.code} <span className="text-muted-foreground">— {s.name}</span></div>
                ...
                <div className="text-[10px] font-mono text-muted-foreground mt-1">{count} units</div>
              </div>
      ...
      <Card className="p-4 bg-card border-border">
        ...
        <form onSubmit={submit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <Field label="Code"><Input value={code} onChange={(e) => setCode(e.target.value)} placeholder="OST" className="font-mono" /></Field>
